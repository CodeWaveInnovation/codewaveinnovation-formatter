...
describe('MaxLineLengthRule', () => {
  ...
  describe('apply with warn action', () => {
    it('should not modify content when action is warn', () => {
      ...
      const config: FormatterConfig = {
        rules: [
          { name: 'max-line-length', enabled: true, options: { length: 80, action: 'warn' } },
        ],
      };
  ...
  describe('apply with wrap action', () => {
    it('should wrap long lines', () => {
      ...
      const config: FormatterConfig = {
        rules: [
          { name: 'max-line-length', enabled: true, options: { length: 80, action: 'wrap' } },
        ],
      };
    ...
    it('should wrap at word boundaries when possible', () => {
      ...
      const config: FormatterConfig = {
        rules: [
          { name: 'max-line-length', enabled: true, options: { length: 50, action: 'wrap' } },
        ],
      };
