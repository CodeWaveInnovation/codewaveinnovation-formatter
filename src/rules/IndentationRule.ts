...
export class IndentationRule extends BaseFormattingRule {
  ...
  protected format(context: FormatContext): string {
    ...
    const indentChar = indentStyle === 'tab' ? '\t' : ' '.repeat(indentSize);
    ...
    const formattedLines = lines.map(line => {
      ...
      const level = Math.floor(leadingSpace.replace(/\t/g, '  ').length / (indentStyle === 'tab' ? 2 : indentSize));
      const restOfLine = line.substring(leadingSpace.length);
