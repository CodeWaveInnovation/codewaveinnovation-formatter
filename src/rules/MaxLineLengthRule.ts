...
export class MaxLineLengthRule extends BaseFormattingRule {
  ...
  protected format(context: FormatContext): string {
    ...
    for (const line of lines) {
      if (line.length <= (maxLength as number)) {
        wrappedLines.push(line);
      } else {
        // Simple word-wrap
        let remaining = line;
        while (remaining.length > (maxLength as number)) {
          let breakPoint = maxLength;
          // Try to break at a space
          const spaceIndex = remaining.lastIndexOf(" ", maxLength as number);
          if (spaceIndex > (maxLength as number) * 0.6) {
            breakPoint = spaceIndex;
          }

          wrappedLines.push(remaining.substring(0, breakPoint as number));
          remaining = remaining.substring(breakPoint as number).trim();
        }
